...
describe("generateStorageKey", () => {
  ...
  it("should generate key with prefix", () => {
    ...
    expect(key).toMatch(
      /^vault-abc\/deviations\/user-123\/artwork---[a-f0-9]{8}\.jpg$/
    );
  });
  ...
  it("should handle nested prefix paths", () => {
    ...
    expect(key).toMatch(
      /^prod\/tenant-456\/deviations\/user-123\/test---[a-f0-9]{8}\.png$/
    );
  });
  ...
  it("should sanitize special characters in filename", () => {
    ...
    expect(key).toMatch(
      /^deviations\/user-123\/my-artwork--final----[a-f0-9]{8}\.jpg$/
    );
  });
